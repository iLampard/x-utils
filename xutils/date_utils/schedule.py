...
class Schedule(object):
    def __init__(self,
        ...
        if effectiveDate is None and firstDate is None and dateGenerationRule == DateGeneration.Backward:
            ...
            py_assert(evalDate < terminationDate, ValueError, "null effective date")
            ...
            py_assert(effectiveDate is not None, ValueError, "null effective date")

        py_assert(effectiveDate < terminationDate, ValueError, "effective date ({0}) "
        ...
        if self._firstDate is not None:
            if self._rule == DateGeneration.Backward or self._rule == DateGeneration.Forward:
                py_assert(effectiveDate < self._firstDate < terminationDate, ValueError,
        ...
        if self._nextToLastDate is not None:
            if self._rule == DateGeneration.Backward or self._rule == DateGeneration.Forward:
                py_assert(effectiveDate < self._nextToLastDate < terminationDate, ValueError,
