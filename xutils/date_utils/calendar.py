...
class Calendar(object):
    def __init__(self, hol_center):
        hol_center = hol_center.lower()
        try:
            self._impl = _holDict[hol_center]()
        except KeyError:
            raise ValueError("{0} is not a valid description of a holiday center".format(hol_center))
        self.name = hol_center
    ...
    def holDatesList(self, fromDate, toDate, includeWeekEnds=True):
        py_assert(fromDate <= toDate, ValueError, "from date ({0} must be earlier than to date {1}"
...
class ChinaSseImpl(CalendarImpl):
    ...
    def isWeekEnd(self, week_day):
        return week_day == Weekdays.Saturday or week_day == Weekdays.Sunday
...
class ChinaIBImpl(CalendarImpl):
    def __init__(self):
        pass
...
class WestenImpl(CalendarImpl):
    ...
    def __eq__(self, right):
        return isinstance(right, WestenImpl)
